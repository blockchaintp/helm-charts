...
* feat(daml-on-besu): add extra volume mounts for the daml statefulset

## v0.2.48

* docs: add daml-on-besu/external-db.md
* fix: remove extra resourced declaration
* feat: changes to allow external postgres database for index
* fix: disable readiness probe
* fix: besu with readiness probes needs to start parallel
* feat(daml-on-sawtooth): use a secret to store the pagerduty keys
* feat(sextant): set the key in an existing postgres password secret that contains the password from values.yaml

## v0.2.47

* feat(daml-on-sawtooth): add pagerduty-alert

## v0.2.46

* feat(daml-on-besu): make jsonapi optional
* fix(daml-on-besu): local bootnodes onlywhen seed
* fix:(daml-on-besu) change besu bootnode strategy
* feat(sawtooth): add pagerduty-alert
* feat(taekion-middleware): add a service for the middleware so we can get to it via k8s api server
* refactor(besu): align filenames b/w besu and daml-on-besu
* feat(besu): add default readiness and liveness probes to besu impls
* feat(daml-on-postgres): add jsonapi
* feat(daml-on-besu): add jsonapi
* feat(daml-on-besu): add annotations option for besu and daml-rpc statefulsets
* feat(besu): bump chart version
* feat(besu): add secondary network capabilities
* fix(daml-on-besu): remove extraneous labels on SS

## v0.2.45

* feat(daml-on-besu): besu child network capabilities
* feat(daml-on-besu): allow arbitrary affinity rules to be set for the statefulset
* feat(besu): allow arbitrary affinity rules to be set for the statefulset

## v0.2.44

* feat(daml-on-besu): update chart version
* docs(daml-on-besu): add multiple network documents
* feat(daml-on-besu): add pod specific services to each besu node to maintain a stable endpoint
* feat(daml-on-besu): add capabilities to connect to a remote besu network

## v0.2.43

* fix: correct cert names in d-o-p and d-o-b
* docs(daml-on-sawtooth): correct values file markdown
* fix(daml-on-sawtooth): remove affinity rule switch and set from daml values
* feat(sawtooth): allow setting arbitrary affinity rules
* feat(daml-on-sawtooth): allow setting arbitrary affinity rules
* test(pre-commit-config): autoupdate hook repos
* fix(daml-on-postgres): correct daml certificate name

## v0.2.42

* fix(daml-on-postgres): corrections to form
* fix(daml-on-postgres): port isn't a dns1123 field, and for now give Host a default
* fix(sawtooth): deprecate seth and sample applications
* fix(sextant-test): correct image urls
* fix(sextant-test): correct values.yaml
* docs(sextant): update sextant documentation

## v0.2.41

* fix(daml-on-postgres): correct extra_args processing
* fix(daml-on-besu): correct extra-args processing

## v0.2.40

* feat(daml-on-sawtooth): enable signals on the remaining containers
* fix(daml-on-sawtooth): correct a typo in the atomicwrite cleanup
